...
export function PublicProfileView({
  ...
  useEffect(() => {
    async function load() {
      ...
      setErrorMessage("");
      try {
        const [p, s, r, a, subs] = await Promise.all([
      ...
      <section className="col-span-12 space-y-4 md:col-span-4">
        ...
        {stats ? (
          <div className="grid grid-cols-2 gap-3">
            ...
            <StatCard label="Solved" value={String(stats.total_solved)} />
            <StatCard label="Easy" value={String(stats.solved_by_difficulty.easy)} />
            <StatCard label="Medium" value={String(stats.solved_by_difficulty.medium)} />
            <StatCard label="Hard" value={String(stats.solved_by_difficulty.hard)} />
            <StatCard label="Accept %" value={`${acceptancePct}%`} />
          </div>
        ) : null}
      </section>
